...
class _HealthProfilePageState extends State<HealthProfilePage> {
  ...
  final _yearsWithConditionController = TextEditingController();
  ...
  final List<String> _glucoseUnitOptions = ['mg/dL', 'mmol/L'];
  ...
  void dispose() {
    ...
    _yearsWithConditionController.dispose();
    ...
    // Menghapus dispose untuk controller yang sudah tidak digunakan
    // _preferredUnitsController.dispose();
    // _glucoseUnitController.dispose();
                ...
                onPressed: () {
                  if (_currentPage < pages.length - 1) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Profil kesehatan disimpan!')),
                    );
  ...
  Widget _buildCoreIdentitySection(bool isAdvanced) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSectionTitle('Kondisi Utama'),
        Wrap(
          spacing: 8.0,
          runSpacing: 8.0,
        ...
        Visibility(
          ...
          child: Column(
            ...
            children: [
              const SizedBox(height: 16),
              _buildSectionTitle('Tanggal Diagnosis'),
              TextFormField(
              ...
              const SizedBox(height: 16),
              _buildSectionTitle('Tahun dengan Kondisi Ini'),
              _buildTextField(
                controller: _yearsWithConditionController,
                hintText: 'Jumlah tahun', keyboardType: TextInputType.number),
            ],
          ),
        ),
      ],
  ...
  Widget _buildBiometricsAndTargetsSection(bool isAdvanced) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSectionTitle('Tinggi & Berat Badan'),
        Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Expanded(child: _buildTextField(controller: _heightController, hintText: 'Tinggi (cm)', keyboardType: TextInputType.number)),
            const SizedBox(width: 16),
            Expanded(child: _buildTextField(controller: _currentWeightController, hintText: 'Berat (kg)', keyboardType: TextInputType.number)),
          ],
        ),
        const SizedBox(height: 16),
        ...
        Visibility(
          ...
          child: Column(
            ...
            children: [
              const SizedBox(height: 24),
              _buildSectionTitle('Target Laboratorium'),
              _buildTextField(controller: _hba1cTargetController, hintText: 'Target HbA1c (%)', keyboardType: TextInputType.number),
              const SizedBox(height: 16),
              _buildTextField(controller: _lastHba1cController, hintText: 'Hasil HbA1c Terakhir (%)', keyboardType: TextInputType.number),
              const SizedBox(height: 16),
              ...
              const SizedBox(height: 16),
              _buildSectionTitle('Target Gula Darah'),
  ...
  Widget _buildActivityLifestyleSection(bool isAdvanced) {
    return Column(
      ...
      children: [
        ...
        _buildTextField(controller: _activityLevelController, hintText: 'Contoh: sedentary, lightly_active'),
        const SizedBox(height: 16),
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8.0,
          runSpacing: 8.0,
          children: _glucoseUnitOptions.map((unit) {
            return ChoiceChip(
              label: Text(unit),
              selected: _selectedGlucoseUnit == unit,
              onSelected: (isSelected) {
                setState(() {
                  if (isSelected) _selectedGlucoseUnit = unit;
                });
              },
              selectedColor: Theme.of(context).colorScheme.primary,
              labelStyle: TextStyle(
                color: _selectedGlucoseUnit == unit ? Colors.white : Colors.black,
              ),
              backgroundColor: Colors.white,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: BorderSide(color: Colors.grey.shade300)),
              checkmarkColor: Colors.white,
